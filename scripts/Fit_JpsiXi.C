...
void Fit_JpsiXi(Int_t run = 1, Bool_t isData = true, Bool_t logFlag = true)
{
	...
	if(isData && logFlag)
	{
		gSystem->RedirectOutput(Form("logs/data/JpsiXi/run%d/Fit_JpsiXi_systematics_new_log.txt",run),"a");//NOTE LOG FILE NAME AND WRITE MODE HAS CHANGED
	}
	else if(!isData && logFlag)
	{
		gSystem->RedirectOutput((Form("logs/mc/JpsiXi/run%d/Fit_JpsiXi_systematics_new_log.txt",run)),"a");//NOTE LOG FILE NAME AND WRITE MODE HAS CHANGED
	}
