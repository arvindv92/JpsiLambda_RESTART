...
void
StandardHypoTestInvDemo_Hypatia(const char * infile = 0,
...
RooStats::HypoTestInvTool::AnalyzeResult( HypoTestInverterResult * r,
   ...
   if (mEnableDetOutput) {
      mWriteResult=true;
      Info("StandardHypoTestInvDemo_Hypatia","detailed output will be written in output result file");
   }
   ...
   if (r != NULL && mWriteResult) {
      ...
      Info("StandardHypoTestInvDemo_Hypatia","HypoTestInverterResult has been written in the file %s",mResultFileName.Data());
...
RooStats::HypoTestInvTool::RunInverter(RooWorkspace * w,
   ...
   if (!sbModel->GetObservables()) {
      Error("StandardHypoTestInvDemo_Hypatia","Model %s has no observables ",modelSBName);
      return 0;
   }
   if (!sbModel->GetSnapshot() ) {
      Info("StandardHypoTestInvDemo_Hypatia","Model %s has no snapshot  - make one using model poi",modelSBName);
   ...
   if (optHTInv.noSystematics) {
      ...
      if (nuisPar && nuisPar->getSize() > 0) {
         std::cout << "StandardHypoTestInvDemo_Hypatia" << "  -  Switch off all systematics by setting them constant to their initial values" << std::endl;
   ...
   if (!bModel || bModel == sbModel) {
      Info("StandardHypoTestInvDemo_Hypatia","The background model %s does not exist",modelBName);
      Info("StandardHypoTestInvDemo_Hypatia","Copy it from ModelConfig %s and set POI to zero",modelSBName);
      ...
      if (!bModel->GetSnapshot() ) {
         Info("StandardHypoTestInvDemo_Hypatia","Model %s has no snapshot  - make one using model poi and 0 values ",modelBName);
         RooRealVar * var = dynamic_cast<RooRealVar*>(bModel->GetParametersOfInterest()->first());
         if (var) {
            double oldval = var->getVal();
            var->setVal(0);
            ...
            Error("StandardHypoTestInvDemo_Hypatia","Model %s has no valid poi",modelBName);
            return 0;
         }
      }
   }
   ...
   if (type != 1 ) {
      ...
      if (hasNuisParam && !hasGlobalObs ) {
         ...
         if (constrPdf) {
            Warning("StandardHypoTestInvDemo_Hypatia","Model %s has nuisance parameters but no global observables associated",sbModel->GetName());
            Warning("StandardHypoTestInvDemo_Hypatia","\tThe effect of the nuisance parameters will not be treated correctly ");
         }
      }
   }
   ...
   std::cout << "StandardHypoTestInvDemo_Hypatia : POI initial value:   " << poi->GetName() << " = " << poi->getVal()   << std::endl;
   ...
   Info("StandardHypoTestInvDemo_Hypatia","Using %s as minimizer for computing the test statistic",
   ...
   if (doFit)  {
      ...
      Info( "StandardHypoTestInvDemo_Hypatia"," Doing a first fit to the observed data ");
      ...
      if (fitres->status() != 0) {
         Warning("StandardHypoTestInvDemo_Hypatia","Fit to the model failed - try with strategy 1 and perform first an Hesse computation");
      ...
      if (fitres->status() != 0)
         Warning("StandardHypoTestInvDemo_Hypatia"," Fit still failed - continue anyway.....");


      poihat  = poi->getVal();
      std::cout << "StandardHypoTestInvDemo_Hypatia - Best Fit value : " << poi->GetName() << " = "
   ...
   if (testStatType == 0) {
      if (!doFit)
         Info("StandardHypoTestInvDemo_Hypatia","Using LEP test statistic - an initial fit is not done and the TS will use the nuisances at the model value");
      else
         Info("StandardHypoTestInvDemo_Hypatia","Using LEP test statistic - an initial fit has been done and the TS will use the nuisances at the best fit value");
   }
   ...
   // else if (type == 3 ) hc = new AsymptoticCalculator(*data, *bModel, *sbModel, true, mAsimovBins);  // for using Asimov data generated with nominal values
      ...
      Error("StandardHypoTestInvDemo_Hypatia","Invalid - calculator type = %d supported values are only :\n\t\t\t 0 (Frequentist) , 1 (Hybrid) , 2 (Asymptotic) ",type);
      return 0;
   }
   ...
   if (testStat == 0) {
      Error("StandardHypoTestInvDemo_Hypatia","Invalid - test statistic type = %d supported values are only :\n\t\t\t 0 (SLR) , 1 (Tevatron) , 2 (PLR), 3 (PLR1), 4(MLE)",testStatType);
      return 0;
   }
   ...
   if (toymcs && (type == 0 || type == 1) ) {
      ...
      if (sbModel->GetPdf()->canBeExtended() ) {
         if (useNumberCounting)   Warning("StandardHypoTestInvDemo_Hypatia","Pdf is extended: but number counting flag is set: ignore it ");
      }
      else {
         // for not extended pdf
         if (!useNumberCounting  )  {
            int nEvents = data->numEntries();
            Info("StandardHypoTestInvDemo_Hypatia","Pdf is not extended: number of events to generate taken  from observed data set is %d",nEvents);
            toymcs->SetNEventsPerToy(nEvents);
         }
         else {
            Info("StandardHypoTestInvDemo_Hypatia","using a number counting pdf");
      ...
      if (data->isWeighted() && !mGenerateBinned) {
         Info("StandardHypoTestInvDemo_Hypatia","Data set is weighted, nentries = %d and sum of weights = %8.1f but toy generation is unbinned - it would be faster to set mGenerateBinned to true\n",data->numEntries(), data->sumEntries());
      }
      ...
      if (mGenerateBinned &&  sbModel->GetObservables()->getSize() > 2) {
         Warning("StandardHypoTestInvDemo_Hypatia","generate binned is activated but the number of observable is %d. Too much memory could be needed for allocating all the bins",sbModel->GetObservables()->getSize() );
      }
   ...
   if (type == 1) {
      ...
      if (bModel->GetNuisanceParameters() || sbModel->GetNuisanceParameters() ) {
         ...
         if (!nuisPdf)  {
            Info("StandardHypoTestInvDemo_Hypatia","No nuisance pdf given for the HybridCalculator - try to deduce  pdf from the model");
         ...
         if (!nuisPdf ) {
            if (bModel->GetPriorPdf())  {
               ...
               Info("StandardHypoTestInvDemo_Hypatia","No nuisance pdf given - try to use %s that is defined as a prior pdf in the B model",nuisPdf->GetName());
            }
            else {
               Error("StandardHypoTestInvDemo_Hypatia","Cannot run Hybrid calculator because no prior on the nuisance parameter is specified or can be derived");
               return 0;
            }
         }
         assert(nuisPdf);
         Info("StandardHypoTestInvDemo_Hypatia","Using as nuisance Pdf ... " );
         nuisPdf->Print();

         const RooArgSet * nuisParams = (bModel->GetNuisanceParameters() ) ? bModel->GetNuisanceParameters() : sbModel->GetNuisanceParameters();
         RooArgSet * np = nuisPdf->getObservables(*nuisParams);
         if (np->getSize() == 0) {
            Warning("StandardHypoTestInvDemo_Hypatia","Prior nuisance does not depend on nuisance parameters. They will be smeared in their full range");
         }
         delete np;
      ...
      if (testStatType != 2 && testStatType != 3)
         Warning("StandardHypoTestInvDemo_Hypatia","Only the PL test statistic can be used with AsymptoticCalculator - use by default a two-sided PL");
   }
   else if (type == 0 ) {
   ...
   if (mRebuild) {
      ...
      std::cout << "StandardHypoTestInvDemo_Hypatia: Initial parameters used for rebuilding: ";
...
void ReadResult(const char * fileName, const char * resultName="", bool useCLs=true) {
   ...
   StandardHypoTestInvDemo_Hypatia(fileName, resultName,"","","",0,0,useCLs);
}


#ifdef USE_AS_MAIN
int main() {
    StandardHypoTestInvDemo_Hypatia();
}
#endif
