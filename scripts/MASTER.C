...
void MASTER()
{
  ...
  for (Int_t i = 0; i <= 3; i++)//loop over isolation BDT versions.
      ...
      if(isData)
	{
	  ...
	  gROOT->ProcessLine(Form("TrainIsolation(%d, %d, \"%s\", %d)",
				  run, trackType, isoVersion, logFlag));
	  gROOT->Reset();
	}
      ...
      for (Int_t j = 0; j <= 1; j++)//loop over isolation BDT configurations
	{
	  ...
	  gROOT->ProcessLine(Form("ApplyIsolation(%d, %d, %d, %d, 1 , \"%s\", %d, %d)",
				  run, isData, mcType, trackType, isoVersion,
				  isoConf, logFlag));
	  gROOT->Reset();
	  ...
	  if(isData)
	    {
	      ...
	      gSystem->cd("/data1/avenkate/JpsiLambda_RESTART/scripts");
	      gROOT->ProcessLine(Form("ApplyIsolation(%d, %d, %d, %d, 2 , \"%s\", %d, %d)",
				      run, isData, mcType, trackType, isoVersion,
				      isoConf, logFlag));
	      gROOT->Reset();
	      ...
	      for(Int_t d = 0; d<=1; d++)
		{
		  ...
		  gROOT->ProcessLine(Form("TrainFinalBDT(%d, %d, \"%s\", %d, %d, %d, %d)",
					  run, trackType, isoVersion, isoConf,
					  isoFlag, logFlag, newFlag));
		  gROOT->Reset();
	      ...
	      for(Int_t k = 0; k <= 1; k++)   //Loop over final BDT configurations.
		{
		  ...
		  gROOT->ProcessLine(Form("ApplyFinalBDT(%d, %d, %d, %d, \"%s\", %d, %d, 1, %d, false, %d, %d)",
					  run, isData, mcType, trackType, isoVersion,
					  isoConf, finalBDTconf, isoFlag, logFlag, newFlag));
		  gROOT->Reset();

		  //Apply final BDT on zeroTracks data/MC
		  gSystem->cd("/data1/avenkate/JpsiLambda_RESTART/scripts");
		  gROOT->ProcessLine(Form("ApplyFinalBDT(%d, %d, %d, %d, \"%s\", %d, %d, 1, %d, true, %d, %d)",
					  run, isData, mcType, trackType, isoVersion,
					  isoConf, finalBDTconf, isoFlag, logFlag, newFlag));
		  gROOT->Reset();
		  ...
		  if(isData)
		    {
		      ...
		      gSystem->cd("/data1/avenkate/JpsiLambda_RESTART/scripts");
		      gROOT->ProcessLine(Form("ApplyFinalBDT(%d, %d, %d, %d, \"%s\", %d, %d, 2, %d, false, %d, %d)",
					      run, isData, mcType, trackType, isoVersion,
					      isoConf, finalBDTconf, isoFlag, logFlag, newFlag));
		      gROOT->Reset();
		      ...
		      gSystem->cd("/data1/avenkate/JpsiLambda_RESTART/scripts");
		      gROOT->ProcessLine(Form("ApplyFinalBDT(%d, %d, %d, %d, \"%s\", %d, %d, 2, %d, true, %d, %d)",
					      run, isData, mcType, trackType, isoVersion,
					      isoConf, finalBDTconf, isoFlag, logFlag, newFlag));
		      gROOT->Reset();
		      ...
		      gROOT->ProcessLine(Form("OptimizeFinalBDT(%d, %d, \"%s\", %d, %d, %d, %d, %d)",
					      run, trackType, isoVersion, isoConf,
					      finalBDTconf, isoFlag, logFlag, newFlag));
		      gROOT->Reset();
		    }
		}
