...
void TrainFinalBDT(Int_t run = 1,Int_t trackType = 3, TString isoVersion = "v1", Int_t isoConf = 1, Bool_t isoFlag = true, Bool_t logFlag = false, Bool_t newFlag = false)
/*
...
*/
{
  ...
  if(isoFlag)
    {
      outfileName = TString::Format("rootFiles/dataFiles/JpsiLambda/run%d/TMVAtraining/iso/TMVALite-JpsiLambda_%s_data_iso%d_%s%s.root",run,type,isoConf,isoVersion.Data(),mynew);
      outputFile = TFile::Open( outfileName, "RECREATE" );
      factory = new TMVA::Factory( TString::Format("TMVAClassificationLite-JpsiLambda%s_dataRun%d_iso%d_%s_%s%s",type,run,isoConf,isoVersion.Data(),mynew), outputFile,
				   "!V:!Silent:Color:!DrawProgressBar:AnalysisType=Classification" );
    }
  else
    {
      outfileName = TString::Format("rootFiles/dataFiles/JpsiLambda/run%d/TMVAtraining/noIso/TMVALite-JpsiLambda%s_data_noIso%s.root",run,type,mynew);
      outputFile = TFile::Open( outfileName, "RECREATE");
      factory = new TMVA::Factory( TString::Format("TMVAClassificationLite-JpsiLambda%s_dataRun%d_noIso%s",type,run,mynew), outputFile,
				   "!V:!Silent:Color:!DrawProgressBar:AnalysisType=Classification" );
    }
