...
// Systematic tests
bool doSys = false;
bool halfBinWidth = false;
bool altFitFunction = false;
bool altBackFunction = false;
bool noReweighting = false;
bool sysNonRes = false;

bool applyHlt = true;

// add Xib- weight, w(eta)
bool applyEtaWeight = false;  // for systematic, def = false   
bool applypTWeight = false;  // for systematic, def = false   
bool MakeWeightFile = false;
bool useNewStrip  = true;
int preScale = 1;

bool applyEtaCutToHb = true;;
bool applypTCutToHb = true;;
...
double lumJpsiL0_Run1 = 2966.48;
double lumJpsiXi_Run1 = 2966.0;
double lumJpsiL0_2016 = 1551.63;
double lumJpsiXi_2016 = 1622.1;
double lumJpsiL0;
double lumJpsiXi;
...
float XibWtPar[2] = {1.0, 0.2};   // linear correction
float XibpTWtPar[2] = {1.0, -0.20};   // linear correction
float BachPiPar[3] = {0.746, 1.30, 2.12};
float JpsiWeightPar[3];// = {0.75, 0.38, 0.0};

float JpsiXiWeightPar[3] = {1.0, 0.0, 0.0};  // was 0.6
//float JpsiXiWeightPar[3] = {0.47, 0.834, 0.0};  // was 0.6
//float JpsiXiWeightPar[3] = {0.0, 4.5, 0.0};
float XiWeightPar[3] = {1.59, -0.35, 0.029};  // pT weight
...
TFile *fsigdata;
TString massExpLb = "Lb_DTF_M_JpsiLConstr";
TString massExpXib = "Xib_DTF_M_JpsiXiLConstr";
...
{
  frame->SetMaximum(1.0*frame->GetMaximum());
  //frame->SetMaximum(35);
...
TCanvas* makeRooPlot(RooAbsPdf* pdf, RooAbsData* rdh, RooRealVar *mvar, int mode, RooRealVar* nS,
  ...
  RooAbsPdf* sig;// = sig1;
  if(mode==0){
    titleX = "#it{#font[12]{J}/#kern[-0.3]{#psi}#Lambda} mass [MeV/#it{c}^{2}]";
    legTitle = "#it{#Lambda_{b}^{0}}#rightarrow #it{#font[12]{J}/#kern[-0.3]{#psi}#Lambda}";
    if(chargeCut<0){
      titleX = "#it{#font[12]{J}/#kern[-0.3]{#psi}#bar{#Lambda}} mass [MeV/#it{c}^{2}]";
      legTitle = "#it{#bar{#Lambda}_{b}^{0}}#rightarrow #it{#font[12]{J}/#kern[-0.3]{#psi}#bar{#Lambda}}";
    }
    
    sig = sig1;
  }else if(mode==1){
     titleX = "#it{#font[12]{J}/#kern[-0.3]{#psi}#Xi^{#font[122]{-}}} mass [MeV/#it{c}^{2}]";
     legTitle = "#it{#Xi_{b}^{#font[122]{-}}}#rightarrow #it{#font[12]{J}/#kern[-0.3]{#psi}#Xi^{#font[122]{-}}}";
     if(chargeCut<0){
       titleX = "#it{#font[12]{J}/#kern[-0.3]{#psi}#bar{#Xi}^{#font[122]{+}}} mass [MeV/#it{c}^{2}]";
       legTitle = "#it{#bar{#Xi}_{b}^{#font[122]{+}}}#rightarrow #it{#font[12]{J}/#kern[-0.3]{#psi}#bar{#Xi}^{#font[122]{+}}}";
     }
  ...
  RooHist* pull_hist = (*plot2).pullHist();
  pull_hist->GetYaxis()->SetTitle(Form("Fit pull / (%d MeV/c^{2})", bw));   
  pdf->plotOn(plot2,Components(comps),LineColor(kRed),LineWidth(2),Name("Signal"),LineStyle(kDashed));
  setFrameAtt(plot2,titleX, titleY);
  plot2->SetMinimum(0.1);
  plot2->SetMaximum(1.0*plot2->GetMaximum());
  plot2->Draw();
  if(mcFit){
    c->SetLogy();
  ...
  myLatex->SetTextSize(0.060);
  if(tag.Contains("year_0") || tag.Contains("year_2") || tag.Contains("year_4")){
    myLatex->DrawLatex(xt,0.78,"#sqrt{s}=7,8 TeV");
  }else{
    myLatex->DrawLatex(xt,0.78,"#sqrt{s}=13 TeV");
  }
  ...
  if(!mcFit){
    TLegend* legend1 = new TLegend(0.62,0.65,0.88,0.88);
  ...
  cout << "*****************************************************************************************************************"<<endl;
  myLatex->SetTextSize(0.050);
  if(tag.Contains("year_0")>0 || tag.Contains("year_1")) myLatex->DrawLatex(0.72,0.62,"(#it{#Xi}^{#font[122]{-}}#rightarrow#it{#Lambda#pi}^{#font[122]{-}}_{L})");
  if(tag.Contains("year_2")>0 || tag.Contains("year_3")) myLatex->DrawLatex(0.72,0.62,"(#it{#Xi}^{#font[122]{-}}#rightarrow#it{#Lambda#pi}^{#font[122]{-}}_{D})");

  myLatex->SetTextSize(0.04);
  if(!forPaper) myLatex->DrawLatex(xt,0.77,y);

  if(plotPull){
    TCanvas* pull = new TCanvas("pull_"+tag,"pull",800,0,600,300);
    pull->SetRightMargin(0.05);
    pull->SetLeftMargin(0.15);
    pull->SetBottomMargin(0.15);
    RooPlot* pull_plot = mvar->frame(Title(""),Bins(nbin),Range(mlow,mhi));
    setFrameAtt(pull_plot,titleX, "Fit Pull"); 
    pull_plot->SetLabelSize(0.05,"X");
    pull_plot->SetLabelSize(0.05,"Y");
    pull_plot->SetTitleSize(0.06,"X");
    pull_plot->SetTitleSize(0.06,"Y");
    pull_plot->SetTitleOffset(0.7,"Y");
    pull_plot->SetTitleOffset(1.1,"X");
    pull_plot->SetTitle("");
    //pull_plot->GetXaxis()->SetTitle(titleX);
    //pull_plot->GetYaxis()->SetTitle("Fit Pull");
    //pull_plot->GetYaxis()->SetTitleFont(myconfigs->m_font);
    pull_plot->SetMinimum(-5); pull_plot->SetMaximum(5);
    //pull_plot->GetXaxis()->SetTitleFont(myconfigs->m_font);
    pull_plot->addPlotable(pull_hist,"P");
    
    TLine* l1 = new TLine(mlow,0,mhi,0); l1->SetLineStyle(2); pull_plot->addObject(l1);
    TLine* l2 = new TLine(mlow,-3,mhi,-3); l2->SetLineStyle(2);pull_plot->addObject(l2);
    TLine* l3 = new TLine(mlow,3,mhi,3); l3->SetLineStyle(2);pull_plot->addObject(l3);
    pull_plot->Draw(); 
    pull->Update();
    if(makePDFPull){
      TString label = "Lb";
      if(mode==1) label = "Xib";
      pull->Print("./Plots/PullPlot_"+label+"_"+suff+".pdf");      
      pull->Print("./Plots/PullPlot_"+label+"_"+suff+".png");      
      pull->Print("./Plots/PullPlot_"+label+"_"+suff+".eps");      
      pull->Print("./Plots/PullPlot_"+label+"_"+suff+".C");      
    }
  }
    cout << "*****************************************************************************************************************"<<endl;    
...
void fillLbMass(){
  ...
  TCanvas *cLam = new TCanvas("cLam","cLambda",800,400);
  cLam->Divide(2,1);
  cLam->cd(1);
...
void setXibDDLparams(){
  ...
  if(altFitFunction && mcFit) return;
  if(altFitFunction){
    if(year=="Run1"){
      sg2L->setVal(20.0); sg2L->setConstant();
      sg2H->setVal(17.2); sg2H->setConstant();
      fr2A->setVal(0.85); fr2A->setConstant();    
    }else if(year=="Run2"){
      sg2L->setVal(22.0); sg2L->setConstant();
      sg2H->setVal(21.0); sg2H->setConstant();
      fr2A->setVal(0.84); fr2A->setConstant();    
    }    
    return;
  }  
  ...
  if(sysNonRes){
    ...
    mg2->setConstant();
    sgscale2->setVal(1.0);
    sgscale2->setConstant();
  }
  
  if(chargeCut!=0){
    sgscale2->setVal(1.12);
    sgscale2->setConstant();
  }
...
void setXibDDDparams(){
  ...
  if(altFitFunction){
    if(year=="Run1"){
      sg2L->setVal(24.0); sg2L->setConstant();
      sg2H->setVal(21.0); sg2H->setConstant();
      fr2A->setVal(0.90); fr2A->setConstant();    
    }else if(year=="Run2"){
      sg2L->setVal(20.0); sg2L->setConstant();
      sg2H->setVal(18.0); sg2H->setConstant();
      fr2A->setVal(0.83); fr2A->setConstant();    
    }
    return;
  }
...
void setLbDDparams(){
  ...
  if(altFitFunction){
    if(year=="Run1"){
      sg1L->setVal(20.5); sg1L->setConstant();
      sg1H->setVal(19.6); sg1H->setConstant();
      fr1A->setVal(0.897); fr1A->setConstant();    
    }else if(year=="Run2"){
      sg1L->setVal(22.6); sg1L->setConstant();
      sg1H->setVal(21.5); sg1H->setConstant();
      fr1A->setVal(0.892); fr1A->setConstant();    
    }
    return;
  }
...
void setLbLLparams(){
  ...
  if(altFitFunction){
    if(year=="Run1"){
      sg1L->setVal(23.0); sg1L->setConstant();
      sg1H->setVal(22.0); sg1H->setConstant();
      fr1A->setVal(0.87); fr1A->setConstant();    
    }else if(year=="Run2"){
      sg1L->setVal(25.0); sg1L->setConstant();
      sg1H->setVal(24.0); sg1H->setConstant();
      fr1A->setVal(0.88); fr1A->setConstant();    
    }
    return;
  }
...
void doXibFitAndPlot(){
  ...
  if(!mcFit){
    suff = "XiLorD_"+year;
    if(chargeCut>0) suff = suff + "_posQ";
    if(chargeCut<0)  suff = suff + "_negQ";
...
void fillLbMassMC(){
  ...
  cLam->Divide(2,1);
  cLam->cd(1);
  chainl->Draw(massExpLb+">>h1",MCLambdabDD_Cut_NoWin);
  cLam->cd(2);
  chainl->Draw(massExpLb+">>h1LL",MCLambdabLL_Cut_NoWin);
...
void fillXibMassMC(){
  ...
  cXib->Divide(2,2);
  cXib->cd(1);
  chainx->Draw(massExpXib+">>h2",Xib_Cut);
  cXib->cd(2);
  chainx->Draw(massExpXib+">>h2L",Xib_Cut&&XiL_Cut);  
  cXib->cd(3);
  chainx->Draw(massExpXib+">>h2D",Xib_Cut&&XiD_Cut);
  cXib->Update();
...
void getLbMassFitFunctions(){
  ...
  sg1scale = new RooRealVar("#sigma_{1B}/#sigma_{1A}","",1.0,0.5,3.5);
  ...
  fr1A = new RooRealVar("f_{1A}","fraction",0.50,0.2,0.95);
  ...
  if(!altFitFunction){
    ...
    sg1scale->setVal(3.0);
    sg1L = new RooRealVar("#sigma_{1L}","",20,8,30);
    sg1H = new RooRealVar("#sigma_{1H}","",20,8,30);
    gauss1A = new RooGaussian("gauss1A","Signal",*mvar1,*mg1,*sg1A);
    gauss1B = new RooBifurGauss("gauss1B","Signal",*mvar1,*mg1,*sg1L,*sg1H);
  ...
  nsig1 = new RooRealVar("nsig1","",12000,0,30000000);
...
void getXibMassFitFunctions(){
  ...
  sgscale2 = new RooRealVar("sf_{mc2}","",1.0,0.6,2.0);//,0.5,1.5);
  sg2A = new RooFormulaVar("sg2A","","@0*@1",RooArgList(*sg2A_mc,*sgscale2));
  sg2scale = new RooRealVar("#sigma_{2B}/#sigma_{2A}","",1.0,0.5,3.5);
  ...
  fr2A = new RooRealVar("f_{2A}","fraction",0.5,0.3,0.95);

  if(!altFitFunction){
    sg2scale->setVal(1.0); sg2scale->setConstant();
    fr2A->setConstant();
    ...
    sig2 = new RooAddPdf("sig2","Signal",RooArgList(*gauss2A,*gauss2B),RooArgList(*fr2A));
  }else{ 
    sg2scale->setVal(3.0);
    sg2L = new RooRealVar("#sigma_{2L}","",20,8,30);
    sg2H = new RooRealVar("#sigma_{2H}","",20,8,30);
    gauss2A = new RooGaussian("gauss2A","Signal",*mvar2,*mg2,*sg2A);
    //gauss2B = new RooGaussian("gauss2B","Signal",*mvar2,*mg2,*sg2B);
    gauss2B = new RooBifurGauss("gauss2B","Signal",*mvar2,*mg2,*sg2L,*sg2H);
    sig2 = new RooAddPdf("sig2","Signal",RooArgList(*gauss2A,*gauss2B),RooArgList(*fr2A));
    //fr2B = new RooRealVar("f_{2B}","fraction",0.3,0.1,0.7);
    //sig2 = new RooAddPdf("sig2","Signal",RooArgList(*gauss2A,*gauss2B,*gauss2C),RooArgList(*fr2A,*fr2B));
  }
  ...
  //comb2 = new RooExponential("comb2","f_{back}",*mvar2,*b2);
  nsig2 = new RooRealVar("nsig2","",12000,0,30000000);
...
void getCuts(){
  ...
  if(!sysNonRes){    
    ...
    Xi_Cut = "abs(Xi_M-L_M+1115.7-1321.9)>10&& abs(Xi_M-L_M+1115.7-1321.9)<20 && Xi_DIRA_ORIVX>0.995 && Xi_ENDVERTEX_CHI2<25 && Xi_TAU>0"; // Use Long Track for pion from Xi- decay
  }
  
  XiL_Cut = "BachPi_TRACK_Type==3&&BachPi_IPCHI2_OWNPV>16"; // Use Long Track for pion from Xi- decay
  //XiL_Cut = "BachPi_TRACK_Type==3&&BachPi_IPCHI2_OWNPV<9"; // Inverted cut
  ...
  if(applypTCutToHb) Xib_Cut = Xib_Cut && "(0.001*Xib_PT<20)";
  //Xib_Cut = Xib_Cut && Xib_Trig && Jpsi_Cut && LambdaLL_Cut && Xi_Cut && (XiL_Cut||XiD_Cut) ;  // LL Lambda
  ...
  //MCXib_Cut_NoWin = Xib_Cut && "(Xib_BKGCAT<20||(Xib_BKGCAT==50&&abs(Xib_DTF_M_JpsiXiLConstr-5795)<300)||(Xib_BKGCAT==60&&abs(Xib_DTF_M_JpsiXiLConstr-5795)<300))";
  MCXib_Cut_NoWin = Xib_Cut && "(abs(Xib_TRUEID)==5132||(Xib_BKGCAT==60&&abs(Xib_DTF_M_JpsiXiLConstr-5795)<300))";
  //MCXib_Cut_NoWin = Xib_Cut && "(Xib_BKGCAT<20)";
  //MCXib_Cut = Xib_Cut && "(Xib_BKGCAT<20||(Xib_BKGCAT==50&&abs(Xib_DTF_M_JpsiXiLConstr-5795)<25)||(Xib_BKGCAT==60&&abs(Xib_DTF_M_JpsiXiLConstr-5795)<25))";
  MCXib_Cut = Xib_Cut && "(abs(Xib_TRUEID)==5132||(Xib_BKGCAT==60&&abs(Xib_DTF_M_JpsiXiLConstr-5795)<35))";
  ...
  if(applypTCutToHb) Lb_Cut = Lb_Cut && "(0.001*Lb_PT<20)";
  ...
  //MCLambdabDD_Cut_NoWin = LambdabDD_Cut&&"(Lb_BKGCAT<20||(Lb_BKGCAT==60&&abs(Lb_DTF_M_JpsiLConstr-5620)<300)||(Lb_BKGCAT==50&&abs(Lb_DTF_M_JpsiLConstr-5620)<300))";
  //MCLambdabLL_Cut_NoWin = LambdabLL_Cut&&"(Lb_BKGCAT<20||(Lb_BKGCAT==60&&abs(Lb_DTF_M_JpsiLConstr-5620)<300)||(Lb_BKGCAT==50&&abs(Lb_DTF_M_JpsiLConstr-5620)<300))"
  MCLambdabDD_Cut_NoWin = LambdabDD_Cut&&"(abs(Lb_TRUEID)==5122||(Lb_BKGCAT==60&&abs(Lb_DTF_M_JpsiLConstr-5620)<300))";
  MCLambdabLL_Cut_NoWin = LambdabLL_Cut&&"(abs(Lb_TRUEID)==5122||(Lb_BKGCAT==60&&abs(Lb_DTF_M_JpsiLConstr-5620)<300))";
  ...
  MCLambdabDD_Cut = LambdabDD_Cut&&"(abs(Lb_TRUEID)==5122||(Lb_BKGCAT==60&&abs(Lb_DTF_M_JpsiLConstr-5620)<35))";
  MCLambdabLL_Cut = LambdabLL_Cut&&"(abs(Lb_TRUEID)==5122||(Lb_BKGCAT==60&&abs(Lb_DTF_M_JpsiLConstr-5620)<35))";
  ...
  cout << Xib_Cut.Print() << endl;
  cout << endl;
  
  cout << LambdabDD_Cut.Print() << endl;
...
void getLooseCuts(){
  ...
  Xib_Cut = Xib_Cut && mCut;
  MCXib_Cut = Xib_Cut && "(Xib_BKGCAT<20||(Xib_BKGCAT==50&&abs(Xib_DTF_M_JpsiXiLConstr-5795)<35)||(Xib_BKGCAT==60&&abs(Xib_DTF_M_JpsiXiLConstr-5795)<35))";
...
void bookXibBaseHistos(){
 ...
 //*********************************
 // Extra hists specifically for Xib
 //*********************************
 ...
 histNames[i1] = "piAngle"; histExp[i1] = "1000*asin(BachPi_PT/BachPi_P)"; hTit[i1] = HB+" angle"; NB[i1]=40; 
 LO[i1]=0.0; HI[i1]=400.0; i1++;
...
void makeLbBasePlots(){
  ...
  // Plot results
  TCanvas* cc0 = new TCanvas("cc0","Lambda_0",1200,800);
  cout << hda[0] << " " << hmc[0] << endl;
  cc0->Divide(3,2);
  cout << hda[0] << " " << hmc[0] << endl;
  cc0->cd(1);
...
void makeXibPlots(){
  ...
  int j,k;
  double mm;
  TCanvas *cf = new TCanvas("cf","BachPi",800,800);
  cf->Divide(2,2);
  for(int i=0;i<nHistsBachPi;i++){
    ...
    mm = hda[j]->GetMaximum();
    hda[j]->GetYaxis()->SetRangeUser(-2,mm);
    hda[j]->Draw("e");
    hmc[j]->Draw("hist,same");  
    if(i==0) legend1->Draw();
  }
  ...
  for(int i=0;i<nHistsXiVtx;i++){
    ...
    j = histXiVtxf + i;
    //cout << " j =  " << j << " " << hTit[j] << endl;
    if(histNames[j].Contains("XiDira")>0){
      hmc[j]->Draw("hist");  
      hda[j]->Draw("e,same");
    }else{      
      hda[j]->Draw("e");
...
void setLbBranchAddresses(){
      ...
      chainlt->SetBranchAddress("J_psi_1S_MC_MOTHER_ID", &J_psi_1S_MC_MOTHER_ID);
...
void setXibBranchAddresses(){
      ...
      */
      ...
      chainxt->SetBranchAddress("J_psi_1S_MC_MOTHER_ID", &J_psi_1S_MC_MOTHER_ID);
...
void getDataFiles(){
  ...
  if(year=="Run1"){
    ...
    if(useNewStrip){
      ...
      fXi = new TFile("/data1/avenkate/JpsiXi/data/total/jpsixi.root");
      treeXi = (TTree*)fXi->Get("MyTuple");
    }
  }
