...
double lumJpsiL0_Run1 = 2962.0;
double lumJpsiXi_Run1 = 2811.0;
...
void getCuts(){
  ...
  //if(year=="Run2") Lambda_Cut  = Lambda_Cut && "p_ProbNNp>0.1";
...
void fillLbHistos(){
  TTree* tree_data  = TreeWithWeight;
  TTree *tree_sim = newtree;  
...
TChain* getLbDDFiles(){
  ...
  TFile *fsigdata = new TFile(filename);
...
TChain* getXibFiles(){
  ...
  // reco'd
  if(!useNewStrip) { 
    TString ids[4] = {"1001","1002","1003","1004"};
  }
...
void makeLbBasePlots(){
  ...
  for(int i=0;i<nHistsR;i++){
    ...
    if(i==0) { 
      ...
      func->SetRange(0.24,0.94);
    }else if(i==1) {
      func->SetRange(0.08,0.66);
...
void makeXibPlots(){
  ...
  int j,k;
  TCanvas *cfff = new TCanvas("cfff","BachPi",1200,400);
  cout << "Made canvas cf" << endl;
  cfff->Divide(3,1);
  cout << "Divide canvas cf" << endl;
  for(int i=0;i<nHistsBachPi;i++){
    cfff->cd(i+1);
    cout << "cd cf " << i+1 << endl;
    cfff->cd(i+1)->SetBottomMargin(bottomMarginOffset);
    cout << "setbottom " << i+1 << endl;
    j = histBachPif + i;
  ...
  for(int i=0;i<nHistsXiVtx;i++){
    ...
    j = histXiVtxf + i;
    //cout << " j =  " << j << " " << hTit[j] << endl;
    maxim = hmc[j]->GetMaximum();
    hmc[j]->GetYaxis()->SetRangeUser(-5,1.1*maxim);
    hmc[j]->Draw("hist");  
    hda[j]->Draw("e,same");
    if(i==0) legend1->Draw();
  }
  ...
  if(makePDF){
    ...
    makePlots(cfff,"BachPi_from_"+parentPart+tag);
