...
double widthsf[nSample] = {1.167, 1.167,1.167,1.167,1.078,1.046};
...
void getXibMassFitStuff(int i, RooRealVar *mmvar, RooAbsReal* mean){
  ...
  sgScale1[i] = new RooRealVar(Form("sgScale1_%d",i),"",widthsf[i]);//1.0,0.5,2.0);//,0.5,1.5);
...
void asymMassFit(){
  ...
  dataXib[0]->append(*dataXib[2]);
  dataXib[1]->append(*dataXib[3]);
  
  for(int j=0; j<nSampleFit; j++){
    if(iFit[j]<0.5 || j==2 || j==3) continue;
  ...
  for(int j=0; j<nSampleFit; j++){
    ...
    if(makePDF2){
      ...
      if(chargeCut>0) file = file + "_Part";
      if(chargeCut<0) file = file + "_antiPart";
