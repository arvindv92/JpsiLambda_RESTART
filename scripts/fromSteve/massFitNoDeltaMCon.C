...
bool defFit = true;
bool useCommonMassXib = false;
bool useCommonMassLb = false;
...
void getMeans(){
  ...
  if(meanOpt==1){
    ...
    for(int i=0; i<4;i++){
      mXib[i] = new RooRealVar(Form("mXib_%d",i),"",5795, 5785, 5805);
...
void massFitNoDeltaMCon(){
  makePDF = false;  // keep false
  ...
  for(int j=0; j<nSampleFit; j++){
    ...
    suff = tags[j]+"_"+tag+"_NoDeltaMCon";
    ...
    if(makePDF2){
      file = "MassFit_NoDeltaMCon_"+tags[j];
